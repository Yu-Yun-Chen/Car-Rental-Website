...
<body>
    ...
    <div class="container">
        <h2>車輛資訊</h2>
        <form name="form" method="post" action="rating_u.php">
            <?php
            include("connect.inc.php");
            $ui = $_SESSION['UserID'];
            ...
            if (!$row[0]){
                echo '<p class="no-car">無車可還！</p>';
                header("refresh:4; url=homepage_u.php"); // Redirect after 2 seconds
                exit;
            }
            $r = @mysqli_fetch_row(mysqli_query($conn, "select locationname from location where locationid = '$row[4]'"));
            $r2 = @mysqli_fetch_row(mysqli_query($conn, "select locationname from location where locationid = '$row[5]'"));
            echo '<p>Start Date:  ' . $row[3] . '</p>';
            echo '<p>End Date:  ' . $row[6] . '</p>';
            echo '<p>Rent Location:  ' . $r[0] . '</p>';
            echo '<p>Return Location:  ' . $r2[0] . '</p>';
            echo '<p>Car Type:  ' . $row2[0] . '</p>';
            ?>
            <input type="hidden" name="vehicleID" value="<?php echo htmlspecialchars($row[2]); ?>">
            <div class="button-container">
				<input type="submit" name="button" value="確定還車"><br>
